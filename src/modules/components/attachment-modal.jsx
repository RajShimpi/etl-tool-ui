...
const AttachmentModal = (props) => {
    ...
    useEffect(() => {
        setAttachmentData(props.attachments ? props.attachments : [])
    }, [props.attachments])
    ...
    const deleteImage = (e, file) => {
        ...
        confirmAlert("Do you want to delete this file?", () => {
            props.onChange(props.attachments.filter(ele => ele.fileName !== file.fileName), props.sequenceNo)
        }, () => { return; })
    }
    ...
    const saveAttachments = () => {
        props.onChange(attachmentData, props.sequenceNo)
    }
            ...
            <div className={`modal-dialog modal-lg`} role="document">
                <div className="modal-content p-2 center-modal">
                    ...
                    <div className="row mt-2 mb-2">
                        {props.isEditable &&
                            <div className={experimentAccess === 'VIEW' || experimentAccess === "REVIEW" ? 'col-xl-2 col-lg-2 col-md-6 col-sm-12 disabled-button' : "col-xl-2 col-lg-2 col-md-6 col-sm-12"}>
                                <a  {...getRootProps()}>
                                    ...
                                    {file.type?.includes("image") ?
                                        <>
                                            {props.isEditable && <div className="overlay">
                                                ...
                                                <button className={experimentAccess === 'VIEW' || experimentAccess === "REVIEW" ? 'icon disabled' : "icon"} id="pdficon" title="Remove" onClick={(e) => deleteImage(e, file)} type='button'>
                                        ...
                                        <>
                                            {props.isEditable && <div className="overlay">
                                                {/*data-bs-toggle="modal" data-bs-target=".bs-example-modal-center"  - this is removed because it is giving backdrop error*/}
                    ...
                    {props.isEditable ?
                        <div className=" col-md-12 d-flex justify-content-end mt-3 mb-3">
                            <button type="button" onClick={() => saveAttachments()} style={{ marginRight: '1%' }}
                                className={experimentAccess === "VIEW" || experimentAccess === "REVIEW" ? "btn mx-1 btn-add w-xs waves-effect waves-light disabled": "btn mx-1 btn-add w-xs waves-effect waves-light"} >
                                <i className="fa fa-plus"></i>
                                Save
                            </button>
                            <button type="button" className={experimentAccess === "VIEW" || experimentAccess === "REVIEW" ? "btn btn-warning w-xs waves-effect waves-light disabled" : "btn btn-warning w-xs waves-effect waves-light" }
