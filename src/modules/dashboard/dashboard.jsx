...
const Dashboard = () => {
    ...
    const getMenusRoutes = () => {
        ...
        let dt1 = menuData.reduce((x, y) => _.concat(x, y.childMenu), []).filter(x => x).map(x => x.href.toLowerCase());
            ...
            <header id="page-topbar">
                <div className="navbar-header">
                    <div className="d-flex">
                        <div className="navbar-brand-box">
                            <Link to="/dashboard" className="logo logo-light">
                                <span className="logo-sm">
                                    <img src="/assets/images/companyLogo.png" alt="" height="40"></img>
                                </span>
                        ...
                        <SearchFilter setExperimentIds={setExperimentIds} searchQuery={searchQuery} setSearchQuery={setSearchQuery} finalData={finalData} setfinalData={setfinalData}  />
                                ...
                                <div className="collapse navbar-collapse" id="topnav-menu-content">
                                    <ul className="navbar-nav" style={{ textAlign: "left" }}>
                                        ...
                                        {menuData.map((item, index) =>
                                            <li key={item.menuId + 'li'} className="nav-item dropdown">
                                                {item.menuName !== 'Dashboard' &&
                                                    <a key={item.menuId + 'anchor'} className="nav-link dropdown-toggle arrow-none" id={item.menuName} role="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                                                        <i key={item.menuId + 'i'} className="uil-apps me-2"></i>{item.menuName}
                                                        <div key={item.menuId + 'div-arrow'} className="arrow-down"></div>
                                                    </a>}
                                                {item.hasChild && (!item.menuName.toLowerCase().includes("master") ? <div key={item.menuId + 'dropdown-div-menu'} className="dropdown-menu" aria-labelledby={item.menuName}>
                                                    {item.childMenu.map((childItem, chidIndex) =>
                                                        <Link key={childItem.childMenuItemId + "Link"} className="dropdown-item" to={{ pathname: childItem.href }} >{childItem.itemName}</Link>
                                                    )}
                                                </div> :
                                                    <div key={item.menuId + 'mega-dropdown-menu'} className="dropdown-menu mega-dropdown-menu px-2 dropdown-mega-menu-xl" aria-labelledby={item.menuName}>
                                                        <div key={item.menuId + 'mega-dropdown-menu-row'} className="row">
                                                            {splitIntoChunk(item.childMenu, 8).map((child, childIn) =>
                                                            (<div key={'mega-dropdown-menu' + childIn} className="col-lg-4">
                                                                {child.map((childItem) => (<div key={childItem.childMenuItemId + "div"}>
                                                                    <Link key={childItem.childMenuItemId + "Link"} className="dropdown-item" to={{ pathname: childItem.href }} >{childItem.itemName}</Link>
                                                                </div>))}
                                                            </div>))}
                                                        </div>
                    ...
                    <div className="d-flex">
                        <div className="dropdown d-inline-block d-lg-none ms-2">
                            <button type="button" className="btn header-item noti-icon waves-effect" id="page-header-search-dropdown" data-bs-toggle="dropdown"
                        ...
                        <div className="dropdown d-none d-lg-inline-block ms-1">
                            <button type="button" className="btn header-item noti-icon waves-effect" data-bs-toggle="fullscreen" onClick={() => { changeScreenMode(!fullScreenMode) }}>
                                <i className="fas fa-expand-arrows-alt"></i>
                            </button>
                        </div>
                        <NotificationsDropdown />
                        <div className="dropdown d-inline-block">
                            <button type="button" className="btn header-item waves-effect d-flex align-items-center" id="page-header-user-dropdown" data-bs-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                                {/* <img className="rounded-circle header-profile-user" src="/assets/images/users/avatar-2.jpg" alt="Header Avatar"></img> */}
                                <Avatar name={auth.getStorageData('fullName')} size="45" round={true} />
                                <span className="d-none d-xl-inline-block ms-1 fw-medium font-size-15"><div><div>{userFirstName}</div></div> {auth.getStorageData('role')}<div className="arrow-down"></div></span>
                            </button>
                            <div className="dropdown-menu dropdown-menu-end">
                                <a className="dropdown-item" href="#"><i className="uil uil-user-circle font-size-18 align-middle text-muted me-1"></i> <span className="align-middle">View Profile</span></a>
                                <a className="dropdown-item d-block" href="#"><i className="uil uil-cog font-size-18 align-middle me-1 text-muted"></i> <span className="align-middle">Settings</span> <span className="badge bg-soft-success rounded-pill mt-1 ms-2">03</span></a>
                                <Link className="dropdown-item" to="/change-password"><i className="uil uil-sign-out-alt font-size-18 align-middle me-1 text-muted"></i> <span className="align-middle">Change Password</span></Link>
                                <Link className="dropdown-item" to="/" onClick={() => logout(true)}><i className="uil uil-sign-out-alt font-size-18 align-middle me-1 text-muted"></i> <span className="align-middle">Sign out</span></Link>
                            </div>
                        </div>
                    </div>
                </div>
